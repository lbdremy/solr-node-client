...
Query.prototype.group = function(options){
   ...
   if(options.facetOn === true){
	   this.parameters.push('group.facet=true');
   }
   return self;
}
...
Query.prototype.facet = function(options){
   ...
   if(options.pivot){
      if(!Array.isArray(options.pivot)){
          options.pivot = [options.pivot];
      }
      for (var i = 0; i < options.pivot.length; i++) {
         this.parameters.push('facet.pivot=' + options.pivot[i])
      }
   }
   return self;
}
