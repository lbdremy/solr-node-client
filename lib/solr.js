 ...
 * 
 * @param {String} file - full path/filename of the file to add to the Solr index
 * @param {Object} [doc] - literal document-fields to add to the index-entry e.g. doc['id']
...
Client.prototype.extractFileContents = function(file,doc,options,callback){
   var stream = fs.createReadStream(file);
   this.extractStreamContents(stream,doc,options,callback);
}

/**
 * Add a Stream's contents to the Solr index through the built-in extraction feature.  
 * Solr will use the Apache Tika library to extract the contents from the uploads.
 * See http://wiki.apache.org/solr/ExtractingRequestHandler. 
 * 
 * @param {Stream} stream - stream producing the content to add to the Solr index
 * @param {Object} [doc] - literal document-fields to add to the index-entry e.g. doc['id']
 * @param {Object} [options] - additional (optional) options to add e.g. options['fmap.content']
 * @param {Function} callback(err,obj) - a function executed when the Solr server responds or an error occurs
 * @param {Error} callback().err
...
Client.prototype.extractStreamContents = function(stream,doc,options,callback){
   if(typeof(options) === 'function'){
      callback = options;
      options = {};
   }
   extr = this.createExtractStream(doc, options);
   var result;
   extr
...
/** Helper Stream to support piping the content to the solr extract handler */
function ExtractHelperStream(onPipeFn) {
  var self = this;
...
util.inherits(ExtractHelperStream, Stream);

/**
 * Create an ExtractStream capable of pipeing content to the Solr extraction handler.  
 * Solr will use the Apache Tika library to extract the contents from the received content
 * See http://wiki.apache.org/solr/ExtractingRequestHandler. 
 * 
 * @param {Object} [doc] - literal document-fields to add to the index-entry e.g. doc['id']
 * @param {Object} [options] - additional (optional) options to add e.g. options['fmap.content']
...
Client.prototype.createExtractStream = function(doc, options) {
   if (doc) {
       options = options || {};
       Object.keys(doc).forEach(function(key) {
           options['literal.'+key] = doc[key];
       });
   }
   ...
   var extractor = new ExtractHelperStream(function(source){
      form.append('my_file', source);
      ...
      var callbackResponse = function(res){
         ...
         res.on('end',function(){
            if(res.statusCode !== 200){
               extractor.emit('error', new SolrError(res.statusCode,buffer));
            }else{
               try{
                  var data = JSONbig.parse(buffer);
                  extractor.emit('data', data);   
               }catch(error){
      ...
      var request = pickProtocol(params.secure).request(options,callbackResponse);
...
Client.prototype.ping = function(callback){
   //ping forces omitHeaders since solr has a duplicate-key json violation in their ping-response according to json-big
