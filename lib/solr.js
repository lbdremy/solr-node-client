 ...
 * @param {String} [handler=''] - path for custom search handler requests
 *
 * @return {Client}
 * @api public
 */
 ...
 * @param {String} options.handler - path for custom search handler requests
 *
 * @return {Client}
 * @api private
 */
...
var Client = function(options){
  this.options = {
    ...
    handler : options.handler || 'select' // default handler for search
  };
  this.data = {};
...
Client.prototype.search = function(query,callback){
  ...
  this.options.fullPath = [this.options.path,this.options.core,this.options.handler + '?' + parameters + '&wt=json']
    .filter(function(element){
      if(element) return true;
      return false;
    })
