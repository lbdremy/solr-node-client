...
Client.prototype.createAddStream = function(options){
   var path = [this.options.path,this.options.core,'update/json?'+ querystring.stringify(options) +'&wt=json']
      .filter(function(element){
         return element;   
      })
      .join('/');
...
Client.prototype.update = function(data,options,callback){
   ...
   this.options.fullPath = [this.options.path,this.options.core,'update/json?'+ querystring.stringify(options) +'&wt=json']
                              .filter(function(element){
                                 return element;    
                              })
                              .join('/');
