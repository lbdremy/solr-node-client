 ...
 *
 * @param {Object} [opt]  - query json representing the Query
 * @param {String} [opt.q = *:*] - query issued against solr
 * @param {Number|String} [opt.start = 0] - where should we start?

 * @param {Number|String} [opt.rows = 0] - how many results?
 * @param {Object} [opt.facet] - facet search against solr
 * @param {Object} [opt.group] - grouped search against solr
 * @return {Query}
 * @api public
 */
Client.prototype.createQuery = function(opt){
  var query;
  ...
  if(typeof opt.facet !== 'undefined' && typeof opt.facet === "object"){
    query.facet(opt.facet);// prepare a facet query
  }
  if(typeof opt.group !== 'undefined' && typeof opt.group === "object"){
    query.group(opt.group);// prepare a group query
  }
  return query;
}
