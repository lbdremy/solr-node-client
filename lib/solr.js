 ...
 *
 * @param {Array} [endpoints=['127.0.0.1:8983']] - Array of host:port
...
function createClient(endpoints, core, path, secure, bigint, pooloptions) {
  var options = (typeof endpoints === 'object') ? endpoints : {
    endpoints: endpoints,
    core: core,
    path: path,
    secure: secure,
 ...
 * @param {Array} options.endpoints - Array of host:port of the Solr servers
...
function Client(options) {
  this.options = {
    endpoints: options.endpoints || ['127.0.0.1:8983'],
    core: options.core || '',
    path: options.path || '/solr',
    bigint: options.bigint || false
  };
  var protocol = options.secure ? https : http;
  this.pool = new Pool(protocol, this.options.endpoints, options.pooloptions);
...
Client.prototype.getJSON = function(params, callback) {
  ...
  var request = self.pool.request(options, function(err, response, body) {
    if (err) {
      callback(err, null);
    } else {
      handleJSONResponse(response, body, params.bigint, callback);
