...
/**
 * Sets the Stats component on the query
 *
 * @param {Object} options - set of options to create a stats component
 * @param {String} [options.field] - The name of the field on which the statistics will be computed. This value should be supplied.
  * @param {String} [options.key] - The alias name forthe field on which the statistics will be computed. If omitted, the field name itself is set as the key.
 * @param {Boolean} [options.min] - If true, the stats result will include the minimum value of the specified field in all documents in the set.
 * @param {Boolean} [options.max] - If true, the stats result will include the maximum value of the specified field in all documents in the set.
 * @param {Boolean} [options.sum] - If true, the stats result will include the sum of all values of the specified field in all documents in the set.
 * @param {Boolean} [options.count] - If true, the stats result will include the number of values found in all documents of the specified field in all documents in the set.
 * @param {Boolean} [options.missing] - If true, the stats result will include The number of documents in the set which do not have a value.
 * @param {Boolean} [options.sumOfSquares] - If true, the stats result will include the sum of all values squared (a by product of computing stddev)
 * @param {Boolean} [options.mean] - If true, the stats result will include the mean of all values of the specified field in all documents in the set.
 * @param {Boolean} [options.stddev] - If true, the stats result will include the standard deviation of all values of the specified field in all documents in the set.
 * @param {Array} [options.percentiles] - The list of all percentile values to be computed and returned in the stats result. The elements can either be integers, float values or strings between 0 to 100
 *
 * @return {Query}
 * @api public
 */
Query.prototype.stats = function(options){
   var self = this;
   var statsClause = [];
   var fieldName = options.field;
   var fieldKey = options.key || fieldName;

   if(options.min){
      statsClause.push('min=true');
   }
   if(options.max){
      statsClause.push('max=true');
   }
   if(options.sum){
      statsClause.push('sum=true');
   }
   if(options.count){
      statsClause.push('count=true');
   }
   if(options.missing){
      statsClause.push('missing=true');
   }
   if(options.sumOfSquares){
      statsClause.push('sumOfSquares=true');
   }
   if(options.mean){
      statsClause.push('mean=true');
   }
   if(options.stddev){
      statsClause.push('stddev=true');
   }
   if(options.percentiles){
      statsClause.push('percentiles=\'' + options.percentiles.join(',') + '\'');
   }

   this.parameters.push('stats=true');
   this.parameters.push('stats.field=' + encodeURIComponent('{!key=' + fieldKey + ' ' + statsClause.join(' ') + '}' + fieldName));
