...
/**
 * Create a facet with global options. To set field specific param use "facetByFieldBasis" function instead.
 *
...
/**
 * Create a facet on a per-field basis.
 *
 * @param {Object} options - set of options to create a facet
 * @param {Boolean} [options.on=true] - Turn on or off facet
 * @param {String} [options.query] - This parameter allows you to specify an arbitrary query in the Lucene default syntax to generate a facet count. By default, faceting returns a count of the unique terms for a "field", while facet.query allows you to determine counts for arbitrary terms or expressions.
...
Query.prototype.facetWithFieldParam = function(options) {
   var self = this;
   if(options.on === false) {
      this.parameters.push('facet=false');
   } else {
  ...
  if(options.field) {
    ...
    if(options.sort) {
      this.parameters.push('f.' + field + '.facet.sort=' + encodeURIComponent(options.sort))
    }
    if(options.limit !== undefined) {
      this.parameters.push('f.' + field + '.facet.limit=' + options.limit);
    }
    if(options.offset !== undefined) {
      this.parameters.push('f.' + field + '.facet.offset=' + options.offset);
    }
    if(options.mincount !== undefined) {
      this.parameters.push('f.' + field + '.facet.mincount=' + options.mincount);
    }
    if(options.missing !== undefined) {
      this.parameters.push('f.' + field + '.facet.missing=' + options.missing);
    }
    if(options.method) {
      this.parameters.push('f.' + field + '.facet.method=' + options.method);
    }
  }
   return self;
};
...
/**
 * Creating a range facet for an specific field.
 *
 * @param {String} [field] - The field to create an range facet with given options. The field name is required and must be set.
 * @param {Object} options - set of options to create a facet
 * @param {Boolean} [options.on=true] - Turn on or off facet
 * @param {String} [options.range.start] - The lower bound of the ranges.
 * @param {String} [options.range.end] - The upper bound of the ranges.
 * @param {String} [options.range.gap] - The size of each range expressed as a value to be added to the lower bound.
 * @param {Boolean} [options.range.hardend] - A Boolean parameter instructing Solr what to do in the event that facet.range.gap does not divide evenly between facet.range.start and facet.range.end. If this is true, the last range constraint will have an upper bound of facet.range.end; if false, the last range will have the smallest possible upper bound greater then facet.range.end such that the range is exactly facet.range.gap wide.
 * @param {String} [options.range.other] - This param indicates that in addition to the counts for each range constraint between facet.range.start and facet.range.end, counts should also be computed for...
 * <ul>
 *  <li>before all records with field values lower then lower bound of the first range
 *  <li>after all records with field values greater then the upper bound of the last range
 *  <li>between all records with field values between the start and end bounds of all ranges
 *  <li>none compute none of this information
 *  <li>all shortcut for before, between, and after
 * </ul>
 * @param {String} [options.range.include] - By default, the ranges used to compute range faceting between facet.range.start and facet.range.end are inclusive of their lower bounds and exclusive of the upper bounds. The "before" range is exclusive and the "after" range is inclusive. This default, equivalent to lower below, will not result in double counting at the boundaries. This behavior can be modified by the facet.range.include param, which can be any combination of the following options...
 * <ul>
 *   <li>lower = all gap based ranges include their lower bound
 *   <li>upper = all gap based ranges include their upper bound
 *   <li>edge = the first and last gap ranges include their edge bounds (ie: lower for the first one, upper for the last one) even if the corresponding upper/lower option is not specified
 *   <li>outer = the "before" and "after" ranges will be inclusive of their bounds, even if the first or last ranges already include those boundaries.
 *   <li>all = shorthand for lower, upper, edge, outer
 * </ul>
 *
 *
 * @return {Query}
 * @api public
 */
Query.prototype.facetByRange = function(field, options) {
   var self = this;
   if(options.on === false) {
      this.parameters.push('facet=false');
   } else {
   ...
   if(field !== undefined) {
      ...
      this.parameters.push('facet.range=' + _field);

      if(options.start) {
         this.parameters.push('f.' + _field + '.facet.range.start=' + options.start)
      }
      if(options.end) {
         this.parameters.push('f.' + _field + '.facet.range.end=' + options.end)
      }
      if(options.gap) {
         this.parameters.push('f.' + _field + '.facet.range.gap=' + options.gap)
      }
      if(options.hardend === true) {
         this.parameters.push('f.' + _field + '.facet.range.hardend=true');
      } else {
         this.parameters.push('f.' + _field + '.facet.range.hardend=false');
      }
      if(options.other) {
         this.parameters.push('f.' + _field + '.facet.range.other=' + options.other)
      }
      if(options.include) {
         this.parameters.push('f.' + _field + '.facet.range.include=' + options.include)
      }
   }
