...
/**
 * Get suggests matching the `query`
 *
 * @param {Query|String} query
 * @param {Function} callback(err,obj) - a function executed when the Solr server responds or an error occurs
 * @param {Error} callback().err
...
Client.prototype.suggest = function(query,callback){
   var self = this;
   var parameters = query.build ? query.build() : query;
   this.options.fullPath = [this.options.path,this.options.core, 'suggest?' + parameters + '&wt=json']
                              .filter(function(element){
                                 if(element) return true;
                                 return false;
                              })
