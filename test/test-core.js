...
suite.addBatch ( {
  'The creation of a Solr Client': {
    ...
    'with custom host, post, core path and search handler': {
      topic: function () {
        var host = 'localhost';
        var port = 8983;
        var core = '';
        var path = '/solr';
        var handler = 'custom';
        var client = solr.createClient ( host, port, core, path, handler );
        return client
      },
      'should return a `Client`': function ( client ) {
        assertClient ( client );
    ...
    'Make a query': {
      ...
      'that return a result where one or more fields are on a range of values': {
        topic: function ( client ) {
          var query = client.createQuery ().q ( 'laptop' ).dismax ().qf ( {title: 2, description: 3} ).start ( 0 ).rows ( 10 ).rangeFilter ( [
            {field: 'price', start: '10', end: '100' },
            {field: 'delievery_t', start: '10', end: '100' }
          ] );
          client.search ( query, this.callback );
        },
        'should be possible': function ( err, res ) {
      ...
      'that autoconvert JS Date Object into a properly date format expected by Solr': {
        topic: function ( client ) {
          ...
          var query = client.createQuery ().q ( 'laptop' ).dismax ().qf ( {title: 2, description: 3} ).start ( 0 ).rows ( 10 ).rangeFilter ( [
            {field: 'last_update', start: start, end: stop },
            {field: 'price', start: '10', end: '100' }
          ] );
          client.search ( query, this.callback );
        },
