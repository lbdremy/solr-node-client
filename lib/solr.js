...
var queryRequest = function(params,callback){
   ...
   if (params.debug){
      debugInfo = {
         requestedUrl : 'http://' + options.host + (options.port != 80 ? ':' + options.port : '') + options.path,
         startDate : new Date()
      };
   ...
   var callbackResponse = function(res){
      ...
      res.on('end',function(){
         if (log){
            debugInfo.endDate = new Date();
            debugInfo.responseTime = debugInfo.endDate - debugInfo.startDate;
         }
         if(res.statusCode !== 200){
            err = new SolrError(res.statusCode,buffer);
            if (log){
