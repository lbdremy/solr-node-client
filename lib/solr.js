...
Client.prototype.get = function(handler,query,callback){
   ...
   var urlLength = [this.options.path,this.options.core,handler,'?',parameters]
                     .reduce(function(length, element){
   ...
   if(urlLength > 1024){
      ...
      var fullPath = [this.options.path,this.options.core,handler + '?' + parameters]
                                 .filter(function(element){
                                    return element;
                                 })
                                 .join('/');
...
function post(params,callback){
   var headers = {
      'content-type' : params.isJson ? 'application/json;' : 'application/x-www-form-urlencoded;' + ' charset=utf-8',
